...
const FormUser = ({id, type, getAllUsers}) => {
    const navigate = useNavigate();
      ...
      const handleSubmit = async (ev) => {
        ...
        try{
              if(type === 'create')
              {
                  const {data} = await axios.post('/api/v1/users/create-user', productData);
        ...
        <form onSubmit={handleSubmit} className="flexStart form">
...

              {
                type === "create" && (
                  <>
                    <div className='w-full'>
                        {preInput('Password', 'create a new password')}
                      <input type="password" className='form_field-input' placeholder='Password' value={password} onChange={(e) => setpassword(e.target.value)} />
                    </div>
